...
public class AiService: IAiService
{
    ...
    public async Task<VulnAiModel?> GenerateVuln(string name, Language language)
    {
     ...
     public async Task<FunctionResult?> GenerateCustom(string prompt)
    {
        try
        {
    ...
    public async Task<FunctionResult?> GenerateExecutive(Project project)
    {
        try
        {
...
This should provide a high-level overview of the key findings and recommendations in a concise and easily understandable manner and the response should be in HTML.  This executive summary should include:
                        ...
                        var membersList = projectUserManager.GetAll().Where(x => x.ProjectId == project.Id).Select(x => x.User.FullName).ToList();
                        string members = "";
                        ...
                        foreach (var vul in vulnsList)
                        {
                            ...
                            vulns += vul.Name + ", Risk: "+ vul.Risk.ToString()+ " , Assets Affected:"+ targetsVuln + ". ";
                        }
                        prompt = message.Replace("{Client}", project.Client.Name).Replace("{Project}", project.Name)
                            .Replace("{StartDate}", project.StartDate.ToShortDateString()).Replace("{EndDate}", project.EndDate.ToShortDateString()).
                            Replace("{ProjectDescription}", project.Description).Replace("{Members}", members).Replace("{Scope}", targets).Replace("{Vulns}", vulns);
    
                        break;
                    case Language.Español:
                        string message2 = @"Eres un pentester que esta redactando el resumen ejecutivo de un informe para un cliente. Este debe proporcionar una descripción general de alto nivel de los hallazgos clave y recomendaciones de manera concisa y fácilmente comprensible y el formato debe ser en HTML. Este resumen ejecutivo debe incluir:
                        ...
                        var membersList2 = projectUserManager.GetAll().Where(x => x.ProjectId == project.Id).Select(x => x.User.FullName).ToList();
                        string members2 = "";
                        ...
                        foreach (var vul in vulnsList2)
                        {
                            ...
                            vulns2 += vul.Name + ", Risk: "+ vul.Risk.ToString()+ " , Assets Affected:"+ targetsVuln + ". ";
                        }
                        prompt = message2.Replace("{Client}", project.Client.Name).Replace("{Project}", project.Name)
                            .Replace("{StartDate}", project.StartDate.ToShortDateString()).Replace("{EndDate}", project.EndDate.ToShortDateString()).
                            Replace("{ProjectDescription}", project.Description).Replace("{Members}", members2).Replace("{Scope}", targets2).Replace("{Vulns}", vulns2);
