...
public class JiraService: IJIraService
{
    ...
    public bool CreateIssue(Guid vuln, string user)
    {
        try
        {
            ...
            var issue = jira.CreateIssue(_jiraConfiguration.Project);
                issue.Type = _jiraConfiguration.TicketConfiguration.IssueType;
                switch (vulnerability.Risk)
                {
                    case VulnRisk.Critical:
                        issue.Priority = _jiraConfiguration.TicketConfiguration.Risk["Critical"];
                        break;
                    case VulnRisk.High:
                        issue.Priority = _jiraConfiguration.TicketConfiguration.Risk["High"];
                        break;
                    case VulnRisk.Medium:
                        issue.Priority = _jiraConfiguration.TicketConfiguration.Risk["Medium"];
                        break;
                    case VulnRisk.Low:
                        issue.Priority = _jiraConfiguration.TicketConfiguration.Risk["Low"];
                        break;
                    case VulnRisk.Info:
                        issue.Priority = _jiraConfiguration.TicketConfiguration.Risk["Info"];
                        break;
                }
                ...
                foreach (var comp in _jiraConfiguration.TicketConfiguration.Components)
                {
                ...
                foreach (var comp in _jiraConfiguration.TicketConfiguration.Labels)
                {
                    components.Append(comp+",");
                }
                ...
                var jiraTicket = new CORE.Entities.Jira
                {
                    VulnId = vuln,
                    UserId = user,
    ...
    public void UpdateIssue(string key)
    {
        ...
        if (issue != null)
        {
            ...
            ticket.JiraType = _jiraConfiguration.TicketConfiguration.IssueType;
    ...
    public async Task<bool> AddCommentAsync(string key, string comment)
    {
        try
        {
            if (!_jiraConfiguration.Enabled)
            {
                return false;
            }
            Atlassian.Jira.Jira jira = null;
            ...
            await issue.AddCommentAsync(com);
            UpdateIssue(key);
            return true;
        }
        catch (Exception e)
        {
            _logger.LogError(e, "An error ocurred in Jira Service deleting Jira Issue : {0}", key);
            return false;
        }
    }
}