...
{
    [Inject] private CveController _cveController { get; set; }
    ...
    private List<CORE.Entities.Cve> cves = new();
    private CveSearchViewModel searchModel = new();
    private CveStatistics statistics = new();
    private string searchString = "";
    private bool loading = true;
    private HashSet<CORE.Entities.Cve> selectedCves = new();
    ...
    protected override async Task OnInitializedAsync()
    {
        breadcrumbs = new List<BreadcrumbItem>
        {
            ...
            new BreadcrumbItem(localizer["cveManagement"], href: null, disabled: true,icon: cveSVG)
        };
    ...
    private async Task LoadData()
    {
        ...
        try
        {
            cves = _cveController.Get().ToList();
            statistics = await _cveController.GetStatisticsForComponentsAsync();
        }
        catch (Exception ex)
        {
            Snackbar.Add($"{localizer["errorLoadingCves"]}: {ex.Message}", Severity.Error);
        }
        finally
        {
            loading = false;
    ...
    private async Task SearchCves()
    {
        ...
        try
        {
            ...
            cves = await _cveController.SearchForComponentsAsync(searchModel);
        }
        catch (Exception ex)
        {
            Snackbar.Add($"{localizer["errorSearchingCves"]}: {ex.Message}", Severity.Error);
        }
        finally
        {
            loading = false;
    ...
    {
        searchModel = new CveSearchViewModel();
        await LoadData();
    }
    ...
    private async Task OpenCreateDialog()
    {
        ...
        IMudExDialogReference<CveCreateDialog>? dlgReference = await DialogService.ShowEx<CveCreateDialog>(localizer["createCve"], parameters, largeWidthEx);
        var result = await dlgReference.Result;

        if (!result.Canceled)
        {
            await LoadData();
            Snackbar.Add(localizer["cveCreatedSuccessfully"], Severity.Success);
        }
    }
    ...
    private async Task TriggerSync()
    {
        ...
        var dialog = await DialogService.ShowAsync<CveSyncDialog>(localizer["syncCves"], parameters, options);
        var result = await dialog.Result;

        if (!result.Canceled && result.Data is CveSyncResult syncResult)
        {
            if (syncResult.IsSuccess)
            {
                var message = $"{localizer["syncCompleted"]} - " +
                            $"{localizer["new"]}: {syncResult.NewCount}, " +
                            $"{localizer["updated"]}: {syncResult.UpdatedCount}, " +
                            $"{localizer["skipped"]}: {syncResult.SkippedCount}";
                
                if (syncResult.ErrorCount > 0)
                {
                    message += $", {localizer["errors"]}: {syncResult.ErrorCount}";
                }

                Snackbar.Add(message, Severity.Success);
                await LoadData(); // Reload the data to show new CVEs
            }
        }
    }

    private void ViewCve(CORE.Entities.Cve cve)
    {
        Navigation.NavigateTo($"/cve/{cve.Id}");
    }

    private async Task ToggleFavorite(CORE.Entities.Cve cve)
    {
        try
        {
            var success = await _cveController.MarkAsFavoriteForComponentsAsync(cve.Id);
            if (success)
            {
                cve.IsFavorite = !cve.IsFavorite;
                var message = cve.IsFavorite ? localizer["addedToFavorites"] : localizer["removedFromFavorites"];
                Snackbar.Add(message, Severity.Success);
            }
            else
            {
                Snackbar.Add(localizer["errorUpdatingFavorite"], Severity.Error);
            }
        }
        catch (Exception ex)
        {
            Snackbar.Add($"{localizer["errorUpdatingFavorite"]}: {ex.Message}", Severity.Error);
        }
    }

    private async Task ArchiveCve(CORE.Entities.Cve cve)
    {
        try
        {
            var success = await _cveController.ArchiveForComponentsAsync(cve.Id);
            if (success)
            {
                cve.IsArchived = !cve.IsArchived;
                var message = cve.IsArchived ? localizer["cveArchived"] : localizer["cveUnarchived"];
                Snackbar.Add(message, Severity.Success);
            }
            else
            {
                Snackbar.Add(localizer["errorArchivingCve"], Severity.Error);
            }
        }
        catch (Exception ex)
        {
            Snackbar.Add($"{localizer["errorArchivingCve"]}: {ex.Message}", Severity.Error);
        }
    }
    ...
    private Func<CORE.Entities.Cve, bool> _quickFilter => element =>
    {
        if (string.IsNullOrWhiteSpace(searchString))
            return true;
        if (element.CveId.Contains(searchString, StringComparison.OrdinalIgnoreCase))
            return true;
        if (element.Title?.Contains(searchString, StringComparison.OrdinalIgnoreCase) == true)
            return true;
        if (element.Description?.Contains(searchString, StringComparison.OrdinalIgnoreCase) == true)
            return true;
        if (element.CvssV3Severity?.Contains(searchString, StringComparison.OrdinalIgnoreCase) == true)
            return true;
        if (element.CvssV3BaseScore?.ToString().Contains(searchString) == true)
            return true;
        if (element.EpssScore?.ToString().Contains(searchString) == true)
            return true;
        if (element.PublishedDate.ToString("yyyy-MM-dd").Contains(searchString))
            return true;
        return false;
    };
    ...
    {
        searchModel.Severities = values.ToList();
    }
    
    private void OnSelectedItemsChanged(HashSet<CORE.Entities.Cve> selected)
    {
        selectedCves = selected;
        StateHasChanged();
    }
    ...
    private async Task DeleteSelectedCves()
    {
        ...
        bool? result = await DialogService.ShowMessageBox(
            ...
            string.Format(localizer["confirmDeleteCvesMessage"], selectedCves.Count),
        ...
        if (result == true)
        {
            try
            {
                var idsToDelete = selectedCves.Select(c => c.Id).ToList();
                await _cveController.DeleteMultipleForComponentsAsync(idsToDelete);
                
                // Remove deleted CVEs from local list
                foreach (var cve in selectedCves.ToList())
                {
                    cves.Remove(cve);
                }
                ...
                Snackbar.Add($"{idsToDelete.Count} {localizer["cvesDeletedSuccessfully"]}", Severity.Success);
            }
            catch (Exception ex)
            {
                Snackbar.Add($"{localizer["errorDeletingCves"]}: {ex.Message}", Severity.Error);
            }
        }
    }
}