...
[Authorize]
public partial class CveNotifications
{


    private List<BreadcrumbItem> breadcrumbs = new();

    private List<CveNotification> notifications = new();
    private List<CveNotification> filteredNotifications = new();
    private CveNotificationStatistics statistics;
    private bool loading = true;
    private string searchString = string.Empty;
    private HashSet<CveNotification> selectedNotifications = new();
    ...
    protected override async Task OnInitializedAsync()
    {
        breadcrumbs = new List<BreadcrumbItem>
        {
            ...
            new BreadcrumbItem(localizer["cveNotifications"], href: null, disabled: true,icon: cveNotSVG),
        };
    ...
    private void FilterNotifications()
    {
        ...
        else
        {
            ...
            filteredNotifications = notifications.Where(n => 
                ...
                n.NotificationType.ToLower().Contains(searchTerm) ||
                n.Priority.ToLower().Contains(searchTerm) ||
                n.Status.ToLower().Contains(searchTerm) ||
                n.Method.ToLower().Contains(searchTerm) ||
                (n.Cve != null && n.Cve.CveId.ToLower().Contains(searchTerm))
            ).ToList();
        }
        StateHasChanged();
    }
    ...
    private async Task LoadData()
    {
        ...
        try
        {
            ...
            notifications = await NotificationManager.GetByUserIdAsync(userId);
            filteredNotifications = notifications.ToList();
            statistics = await NotificationManager.GetStatisticsAsync(userId);
            selectedNotifications.Clear();
        }
        catch (Exception ex)
        {
            Snackbar.Add($"{localizer["errorLoadingNotifications"]}: {ex.Message}", Severity.Error);
        }
        finally
        {
            loading = false;
    ...
    private async Task MarkAsRead(CveNotification notification)
    {
        try
        {
            var authState = await AuthenticationStateProvider.GetAuthenticationStateAsync();
            ...
            await NotificationManager.MarkAsReadAsync(notification.Id, userId);
            ...
            StateHasChanged();
            Snackbar.Add(localizer["notificationMarkedAsRead"], Severity.Success);
        }
        catch (Exception ex)
        {
            Snackbar.Add($"{localizer["errorMarkingNotificationAsRead"]}: {ex.Message}", Severity.Error);
        }
    }
    ...
    private async Task MarkAllAsRead()
    {
        try
        {
            ...
            var count = await NotificationManager.MarkAllAsReadAsync(userId);
            ...
            StateHasChanged();
            Snackbar.Add($"{count} {localizer["notificationsMarkedAsRead"]}", Severity.Success);
        }
        catch (Exception ex)
        {
            Snackbar.Add($"{localizer["errorMarkingAllNotificationsAsRead"]}: {ex.Message}", Severity.Error);
        }
    }

    private void ViewCve(Guid cveId)
    {
        Navigation.NavigateTo($"/cve/{cveId}");
    }

    private void ViewSubscription(Guid subscriptionId)
    {
        Navigation.NavigateTo($"/cve/subscriptions/{subscriptionId}/statistics");
    }
    ...
    private Color GetNotificationTypeColor(string notificationType)
    {
        ...
        {
            "NewCve" => Color.Info,
            "UpdatedCve" => Color.Warning,
            "CriticalCve" => Color.Error,
            "KevAdded" => Color.Error,
            _ => Color.Default
        };
    }
    ...
    private Func<CveNotification, int, string> _rowClassFunc = (item, index) =>
    {
        return item.IsRead ? "notification-read" : "notification-unread";
    };
    
    private Func<CveNotification, int, string> _rowStyleFunc = (item, index) =>
    {
        return item.IsRead ? "opacity: 0.8;" : string.Empty;
    };
    
    private void OnSelectedItemsChanged(HashSet<CveNotification> selected)
    {
    ...
    private async Task DeleteSelectedNotifications()
    {
        ...
        bool? result = await DialogService.ShowMessageBox(
            ...
            string.Format(localizer["confirmDeleteNotificationsMessage"], selectedNotifications.Count),
        ...
        if (result == true)
        {
            try
            {
                var idsToDelete = selectedNotifications.Select(n => n.Id).ToList();
                await NotificationManager.DeleteMultipleAsync(idsToDelete);
                
                // Remove deleted notifications from local lists
                foreach (var notification in selectedNotifications.ToList())
                {
                ...
                if (statistics != null)
                {
                    var deletedUnreadCount = selectedNotifications.Count(n => !n.IsRead);
                    statistics.UnreadNotifications -= deletedUnreadCount;
                    statistics.TotalNotifications -= selectedNotifications.Count;
                }
                
                selectedNotifications.Clear();
                StateHasChanged();
                
                Snackbar.Add($"{idsToDelete.Count} {localizer["notificationsDeletedSuccessfully"]}", Severity.Success);
            }
            catch (Exception ex)
            {
                Snackbar.Add($"{localizer["errorDeletingNotifications"]}: {ex.Message}", Severity.Error);
            }
        }
    }
}