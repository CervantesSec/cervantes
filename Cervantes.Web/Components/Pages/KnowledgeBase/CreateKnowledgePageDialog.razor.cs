...
public partial class CreateKnowledgePageDialog: ComponentBase
{
    ...
    KnowledgePageCreateViewModel model = new KnowledgePageCreateViewModel();
    private List<KnowledgeBaseCategories> Categories = new List<KnowledgeBaseCategories>();
    private Dictionary<string, object> editorConf = new Dictionary<string, object>{
        {"plugins", "preview importcss searchreplace autolink autosave save directionality code visualblocks visualchars fullscreen image link media codesample table charmap pagebreak nonbreaking anchor insertdatetime advlist lists wordcount help charmap quickbars emoticons textpattern"},
    ...
    {
        model.Order = _KnowledgeBaseController.GetPages().Count() + 1;
	 ...
	 private async Task Submit()
    {
        ...
        if (form.IsValid)
        {
            ...
            {
                Snackbar.Add(@localizer["pageCreatedError"], Severity.Error);
            }
    ...
    public class KnowledgeFluentValidator : AbstractValidator<KnowledgePageCreateViewModel>
    {
        ...
        {
            var result = await ValidateAsync(ValidationContext<KnowledgePageCreateViewModel>.CreateWithOptions((KnowledgePageCreateViewModel)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
