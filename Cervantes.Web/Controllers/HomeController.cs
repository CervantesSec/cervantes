        ...
        public IActionResult Index()
        {
            try
            {
                DashboardViewModel model = new DashboardViewModel
                {
                    ...
                    ActiveProjects = projectManager.GetAll().Where(x => x.Status == CORE.ProjectStatus.Active).Take(5).OrderByDescending(x => x.EndDate),
                    RecentClients = clientManager.GetAll().OrderByDescending(x => x.Id).Take(5).OrderByDescending(x => x.CreatedDate),
                    RecentDocuments = documentManager.GetAll().OrderByDescending(x => x.Id).Take(5).OrderByDescending(x => x.CreatedDate),
                    RecentVulns = vulnManager.GetAll().OrderByDescending(x => x.Id).Take(5).OrderByDescending(x => x.CreatedDate),
