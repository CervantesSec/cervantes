    ...
    public class ProjectController : Controller
    {
        ...
        public ActionResult Create(ProjectViewModel model)
        {
            try
            {
                ...
                _logger.LogInformation("User: {0} Created a new Project: {1}", User.FindFirstValue(ClaimTypes.Name), project.Name);
                return RedirectToAction("Index");
            }
            catch (Exception ex)
            {
        ...
        public IActionResult AddTarget(IFormCollection form)
        {
            try
            {
                if (form != null)
                {
                    ...
                    _logger.LogInformation("User: {0} Added new target: {1} on Project: {2}", User.FindFirstValue(ClaimTypes.Name), target.Name, Int32.Parse(form["project"]));
        ...
        public IActionResult AddNote(IFormCollection form)
        {
            try
            {
                if (form != null)
                {
                    ...
                    _logger.LogInformation("User: {0} Added new note: {1} on Project: {2}", User.FindFirstValue(ClaimTypes.Name), note.Name, Int32.Parse(form["project"]));
                    return RedirectToAction("Details", "Project", new { id = Int32.Parse(form["project"]) });
                }
                else
                {
        ...
        public IActionResult DeleteNote(int id, int project)
        {
            try
            {
                if (id != 0)
                {
                    ...
                    _logger.LogInformation("User: {0} Deleted note: {1} on Project: {2}", User.FindFirstValue(ClaimTypes.Name), result.Name, project);
                    return RedirectToAction("Details", "Project", new { id = project });
                }
                else
                {
        ...
        public IActionResult AddAttachment(IFormCollection form, IFormFile upload)
        {
            try
            {
                if (form != null && upload != null)
                {
                    ...
                    _logger.LogInformation("User: {0} Added an attachment: {1} on Project: {2}", User.FindFirstValue(ClaimTypes.Name), note.Name, Int32.Parse(form["project"]));
                    return RedirectToAction("Details", "Project", new { id = Int32.Parse(form["project"]) });
                }
                else
                {
                    TempData["errorAttachment"] = "added";
                    _logger.LogError("An error ocurred adding an Attachment on Project: {1}. User: {2}", Int32.Parse(form["project"]), User.FindFirstValue(ClaimTypes.Name));
                    return RedirectToAction("Details", "Project", new { id = Int32.Parse(form["project"]) });
                }
            }
            catch (Exception ex)
        ...
        public IActionResult DeleteAttachment(int id, int project)
        {
            try
            {
                if (id != 0)
                {
                    ...
                    _logger.LogInformation("User: {0} Deleted an attachment: {1} on Project: {2}", User.FindFirstValue(ClaimTypes.Name), result.Name, project);
                    return RedirectToAction("Details", "Project", new { id = project });
                }
                else
                {
                    _logger.LogError("An error ocurred deleting an Attachment on Project: {1}. User: {2}", project, User.FindFirstValue(ClaimTypes.Name));
                    return RedirectToAction("Details", "Project", new { id = project });
                }
            }
            catch (Exception ex)
